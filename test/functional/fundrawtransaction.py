...
class RawTransactionsTest(BitcoinTestFramework):
    ...
    def run_test(self):
        ...
        self.sync_all()
        self.nodes[0].generate(149)
        self.sync_all()

        # ensure that setting changePosition in fundraw with an exact match is handled properly
        rawmatch = self.nodes[2].createrawtransaction([], {self.nodes[2].getnewaddress():Decimal('67.6000')})
        rawmatch = self.nodes[2].fundrawtransaction(rawmatch, {"changePosition":1, "subtractFeeFromOutputs":[0]})
        assert_equal(rawmatch["changepos"], -1)

        watchonly_address = self.nodes[0].getnewaddress()
        watchonly_pubkey = self.nodes[0].validateaddress(watchonly_address)["pubkey"]
        watchonly_amount = Decimal(1200)
        ...
        assert_raises_rpc_error(-5, "changeAddress must be a valid stash address", self.nodes[2].fundrawtransaction, rawtx, {'changeAddress':'foobar'})
        ...
        self.nodes[0].generate(1)
        self.sync_all()
        assert_equal(oldBalance+Decimal('67.7900000'), self.nodes[0].getbalance()) #0.19+block reward
        ...
        result2 = self.nodes[3].fundrawtransaction(rawtx, {"feeRate": 5*min_relay_tx_fee})
        result3 = self.nodes[3].fundrawtransaction(rawtx, {"feeRate": 10*min_relay_tx_fee})
        result_fee_rate = result['fee'] * 1000 / count_bytes(result['hex'])
        assert_fee_amount(result2['fee'], count_bytes(result2['hex']), 5 * result_fee_rate)

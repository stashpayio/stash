...
# stash_hash
RUN git clone https://github.com/stashpayio/stash_hash
RUN cd stash_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} stash
RUN useradd -u ${USER_ID} -g stash -s /bin/bash -m -d /stash stash
...
RUN mkdir /stash-src && \
  ...
  chown $USER_ID:$GROUP_ID /stash-src && \

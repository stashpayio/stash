...
// which are part of the Stash repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="stash-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /stash-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "stashpay-stash/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from stashpay-stash/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'stashpay-stash/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from stashpay-stash/develop"
            }
          }

          if (hasCache) {
            sh "cd /stash-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /stash-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /stash-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /stash-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /stash-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /stash-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /stash-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }

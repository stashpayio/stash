...
bool AppInitParameterInteraction()
{
    ...
    fExperimentalMode = gArgs.GetArg("-experimentalfeatures", false);

    // Fail early if user has set experimental options without the global flag
    if (!fExperimentalMode) {
        if (gArgs.IsArgSet("-developerencryptwallet")) {
...
bool AppInitMain(boost::thread_group& threadGroup, CScheduler& scheduler)
{
    ...
    if(fLiteMode) {
        InitWarning(_("You are starting in lite mode, most Stash-specific functionality is disabled."));
    }
    ...
    if (gArgs.IsArgSet("-setuplegacyblocks")) {
		setupLegacyBlocks();
        return false;
    }
    ...
    // ********************************************************* Step 10c: schedule Stash-specific tasks

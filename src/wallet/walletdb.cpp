...
{
    return WriteTx(wtx.GetHash(), wtx);
}
                                 ...
                                 const CKeyMetadata &keyMeta)
{
    ...
    if (!WriteIC(std::make_pair(std::string("zkeymeta"), addr), keyMeta))
        return false;

    if (!WriteIC(std::make_pair(std::string("czkey"), addr), std::make_pair(rk, vchCryptedSecret), false))
        return false;
    if (fEraseUnencryptedKey)
    {
        EraseIC(std::make_pair(std::string("zkey"), addr));
    }
    return true;
}
...
bool CWalletDB::WriteZKey(const libzcash::PaymentAddress& addr, const libzcash::SpendingKey& key, const CKeyMetadata &keyMeta)
{
    ...
    return WriteIC(std::make_pair(std::string("zkey"), addr), key, false);
}

bool CWalletDB::WriteViewingKey(const libzcash::ViewingKey &vk)
{
    return WriteIC(std::make_pair(std::string("vkey"), vk), '1');
}

bool CWalletDB::EraseViewingKey(const libzcash::ViewingKey &vk)
{
    return EraseIC(std::make_pair(std::string("vkey"), vk));
}
...
public:
    ...
    CWalletScanState() {
    	  nKeys = nCKeys = nWatchKeys = nKeyMeta = nZKeys = nCZKeys = nZKeyMeta = 0;
        fIsEncrypted = false;
        fAnyUnordered = false;
        nFileVersion = 0;
    }

...
/*
    ...
    return result;
*/
    return signrawtransaction(request.params);
}
...
UniValue sendrawtransaction(const UniValue& params) {
  ...
  if (!fHaveMempool && !fHaveChain) {
      ...
      bool fMissingInputs;
      if (!AcceptToMemoryPool(mempool, state, std::move(tx), !fBypassLimits, &fMissingInputs, false, false, nMaxRawTxFee)) {
          if (state.IsInvalid()) {
...
UniValue sendrawtransaction(const JSONRPCRequest& request)
{
    ...
    return sendrawtransaction(request.params);
}

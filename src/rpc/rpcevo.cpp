...
#ifdef ENABLE_WALLET
extern UniValue signrawtransaction(CWallet* pwallet, const UniValue& request);
extern UniValue sendrawtransaction(CWallet* pwallet, const UniValue& request);
#endif//ENABLE_WALLET
...
std::string GetHelpString(int nParamNum, std::string strParamName)
{
    static const std::map<std::string, std::string> mapParamHelp = {
        {"collateralAddress",
            "%d. \"collateralAddress\"        (string, required) The stash address to send the collateral to.\n"
        },
        {"collateralHash",
        ...
        {"ownerAddress",
            "%d. \"ownerAddress\"             (string, required) The stash address to use for payee updates and proposal voting.\n"
        ...
        {"payoutAddress_register",
            "%d. \"payoutAddress\"            (string, required) The stash address to use for masternode reward payments.\n"
        },
...
// Allows to specify Stash address or priv key. In case of Stash address, the priv key is taken from the wallet
...
static std::string SignAndSendSpecialTx(CWallet* pwallet, const CMutableTransaction& tx)
{
    LOCK(cs_main);
    ...
    CDataStream ds(SER_NETWORK, PROTOCOL_VERSION);
    ds << tx;
    UniValue signRequest;
    signRequest.setArray();
    signRequest.push_back(HexStr(ds.begin(), ds.end()));
    UniValue signResult = signrawtransaction(pwallet, signRequest);

    UniValue sendRequest;
    sendRequest.setArray();
    sendRequest.push_back(signResult["hex"].get_str());
    return sendrawtransaction(pwallet, sendRequest).get_str();
}
...
void protx_register_fund_help(CWallet* const pwallet)
{
    throw std::runtime_error(
            ...
            "\nCreates, funds and sends a ProTx to the network. The resulting transaction will move 1000 Stash\n"
...
UniValue protx_register(const JSONRPCRequest& request)
{
    ...
    if (request.params.size() > paramIdx + 6) {
        ...
        if (!fundAddress.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Stash address: ") + request.params[paramIdx + 6].get_str());
    }
...
UniValue protx_update_service(const JSONRPCRequest& request)
{
    ...
    if (request.params.size() >= 6) {
        ...
        if (!feeSourceAddress.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Stash address: ") + request.params[5].get_str());
...
UniValue protx_revoke(const JSONRPCRequest& request)
{
    ...
    if (request.params.size() > 4) {
        ...
        if (!feeSourceAddress.IsValid())
            throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, std::string("Invalid Stash address: ") + request.params[4].get_str());

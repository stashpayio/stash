...
UniValue dtg1(const JSONRPCRequest& request) {
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2) {
...
UniValue dtg2(const JSONRPCRequest& request) {
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2) {
...
UniValue dtg3(const JSONRPCRequest& request) {
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2) {
...
UniValue dtg4(const JSONRPCRequest& request) {
    if (request.fHelp || request.params.size() < 1 || request.params.size() > 2) {
...
UniValue dtg5(const JSONRPCRequest& request) {
    throw runtime_error("DTG5: No help here!");
}

UniValue dtg6(const JSONRPCRequest& request) {
...
#ifndef DTG
static const CRPCCommand dtgCommands[] =
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
  { "hidden",             "dtg1",                     &dtg1,                    true, {} },
  { "hidden",             "dtg2",                     &dtg2,                    true, {} },
  { "hidden",             "dtg3",                     &dtg3,                    true, {} },
  { "hidden",             "dtg4",                     &dtg4,                    true, {} },
  { "hidden",             "dtg5",                     &dtg5,                    true, {} },
  { "hidden",             "dtg6",                     &dtg6,                    true, {} },
};

void RegisterDTGRPCCommands(CRPCTable &t)
{
    for (unsigned int vcidx = 0; vcidx < ARRAYLEN(dtgCommands); vcidx++)
        t.appendCommand(dtgCommands[vcidx].name, &dtgCommands[vcidx]);
}
#endif

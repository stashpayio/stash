...
UniValue gobject_prepare(const JSONRPCRequest& request)
{
    ...
    // This command is dangerous because it consumes 500 STASH irreversibly.
    // If params are lost, it's very hard to bruteforce them and yet
    // users ignore all instructions on stashcentral etc. and do not save them...
...
void gobject_vote_conf_help()
{
    throw std::runtime_error(
                ...
                "Vote on a governance object by masternode configured in stash.conf\n"
                "\nArguments:\n"
...
UniValue gobject_vote_conf(const JSONRPCRequest& request)
{
    ...
    if (!fMnFound) {
        ...
        resultsObj.push_back(Pair("stash.conf", statusObj));
...
[[ noreturn ]] void gobject_help()
{
    throw std::runtime_error(
            ...
            "  vote-conf          - Vote on a governance object by masternode configured in stash.conf\n"
...
{ //  category              name                      actor (function)         okSafe argNames
  //  --------------------- ------------------------  -----------------------  ------ ----------
    /* Stash features */
    { "stash",               "getgovernanceinfo",      &getgovernanceinfo,      true,  {} },
    { "stash",               "getsuperblockbudget",    &getsuperblockbudget,    true,  {"index"} },
    { "stash",               "gobject",                &gobject,                true,  {} },
    { "stash",               "voteraw",                &voteraw,                true,  {} },

...
struct MainSignalsInstance {
    ...
    boost::signals2::signal<void (const uint256 &hash)> EraseFromWallet;
    boost::signals2::signal<void (const CBlockIndex *pindex, const std::shared_ptr<const CBlock>& pblock, ZCIncrementalMerkleTree tree, bool added)> ChainTip;
...
void CMainSignals::EraseTransaction(const uint256 &hash) {
        m_internals->EraseFromWallet(hash);
}

void CMainSignals::ChainTip(const CBlockIndex *pindex, const std::shared_ptr<const CBlock>& pblock, ZCIncrementalMerkleTree tree, bool added) {
        m_internals->ChainTip(pindex, pblock, tree, added);
}

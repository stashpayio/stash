                                     ...
                                     const CAmount& nAbsurdFee, std::vector<COutPoint>& coins_to_uncache, bool fDryRun)
{
    ...
    if (!CheckTransaction(tx, state, verifier) || !ContextualCheckTransaction(tx, state, Params().GetConsensus(), chainActive.Tip()))
        return false;
    ...
    for (const CTxIn &txin : tx.vin)
    {
        ...
        if (itConflicting != pool.mapNextTx.end())
        {
            ...
            // Transaction conflicts with mempool and RBF doesn't exist in Stash
...
{
    CAmount nSubsidyBase = 6760000000ULL; // 67.6 * COIN;
    /* START STASH
...
{
    LOCK(cs_main);
// START STASH - ToDo Check this is correct
...
static DisconnectResult DisconnectBlock(const CBlock& block, const CBlockIndex* pindex, CCoinsViewCache& view)
{
    ...
    // TODO Harden to DIP0003Height after SPORK_15_DETERMINISTIC_MNS_ENABLED has been set
...
static unsigned int GetBlockScriptFlags(const CBlockIndex* pindex, const Consensus::Params& consensusparams) {
    ...
    // Allways enforce BIP147 (NULLDUMMY)
...
static int64_t nTimeStashSpecific = 0;
...
*/
    ...
    if (sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        for (const auto& tx : block.vtx) {
            ...
            if (llmq::chainLocksHandler->HasChainLock(pindex->nHeight, pindex->GetBlockHash())) {
                ...
                LOCK(cs_main);
                return state.DoS(10, error("ConnectBlock(STASH): transaction %s conflicts with transaction lock %s", tx->GetHash().ToString(), conflictLock->txid.ToString()),
                                 REJECT_INVALID, "conflict-tx-lock");
            }
        }
    } else {
        LogPrintf("ConnectBlock(STASH): spork is off, skipping transaction locking checks\n");
    }
    ...
    LogPrint(BCLog::BENCHMARK, "      - IS filter: %.2fms [%.2fs]\n", 0.001 * (nTime5_1 - nTime4), nTimeISFilter * 0.000001);
        ...
        // STASH : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!ProcessSpecialTxsInBlock(block, pindex, state, fJustCheck, fScriptChecks)) {
        return error("ConnectBlock(STASH): ProcessSpecialTxsInBlock for block %s failed with %s",
    ...
    int64_t nTime5 = GetTimeMicros(); nTimeStashSpecific += nTime5 - nTime4;
    LogPrint(BCLog::BENCHMARK, "    - Stash specific: %.2fms [%.2fs]\n", 0.001 * (nTime5 - nTime4), nTimeStashSpecific * 0.000001);
...
void static UpdateTip(CBlockIndex *pindexNew, const CChainParams& chainParams) {
    ...
    double syncProgress = GuessVerificationProgress(chainParams.TxData(), chainActive.Tip());
...
static bool ContextualCheckBlockHeader(const CBlock& block, CValidationState& state, const CChainParams& params, const CBlockIndex* pindexPrev, int64_t nAdjustedTime)
{
...
// END STASH */
    ...
    // 21/08/2018 This code is no longer required as this low block version do not appear on our blockchain
...
bool initGenesisBlock(const CChainParams& chainparams)
{
    ...
    assert(chainActive.Genesis() == nullptr);
    try {
          CValidationState state;
...
void setupLegacyBlocks() {
    ...
    const auto filenames = gArgs.GetArgs("-setuplegacyblocks");

    std::string coinbaseReward = gArgs.GetArg("-legacyblockreward","6760000000");
    CAmount reward = std::stoll(coinbaseReward);

    if (!gArgs.IsArgSet("-legacyblockpayee")) {
      fprintf(stderr,"**********************************************************************\n");
      fprintf(stderr,"* Legacy block payee is required. (Use -legacyblockpayee parameter.) *\n");
      fprintf(stderr,"**********************************************************************\n");
      return;
    }
    std::string payee = gArgs.GetArg("-legacyblockpayee","");

...
static DisconnectResult DisconnectBlock(const CBlock& block, CValidationState& state, const CBlockIndex* pindex, CCoinsViewCache& view)
{
    ...
    view.PopAnchor(blockUndo.old_tree_root); // MPB Check
...
/*
    21/08/2018: Since BIP34 is enfoced from block 1, it was not possible to create new duplicate coinbases.
            ...
            for (const auto& tx : block.vtx) {
                ...
                for (const auto& txin : tx->vin) {
                    ...
                    if (instantsend.GetLockedOutPointTxHash(txin.prevout, hashLocked) && hashLocked != tx->GetHash()) {
                        ...
                        return state.DoS(10, error("ConnectBlock(STASH): transaction %s conflicts with transaction lock %s", tx->GetHash().ToString(), hashLocked.ToString()),
...
bool static DisconnectTip(CValidationState& state, const CChainParams& chainparams)
{
    ...
    for (const auto& tx : block.get()->vtx) { // MPB ?

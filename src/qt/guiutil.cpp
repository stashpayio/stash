...
static const std::map<ThemedColor, QColor> themedColors = {
    ...
    { ThemedColor::NEGATIVE, QColor(247, 68, 68) },
...
static const std::map<ThemedStyle, QString> themedStyles = {
    ...
    { ThemedStyle::TS_ERROR, "color:#f74444;" },
...
void setupAddressWidget(QValidatedLineEdit *widget, QWidget *parent)
{
    ...
    widget->setPlaceholderText(QObject::tr("Enter a Stash address (e.g. %1)").arg(
...
{
    // return if URI is not valid or is no stash: URI
    if(!uri.isValid() || uri.scheme() != QString("stash"))
        return false;
    ...
    for (QList<QPair<QString, QString> >::iterator i = items.begin(); i != items.end(); i++)
    {
        ...
        else if (i->first == "amount")
        {
            if(!i->second.isEmpty())
            {
                if(!BitcoinUnits::parse(BitcoinUnits::STASH, i->second, &rv.amount))
                {
                    return false;
                }
            }
...
{
    // Convert stash:// to stash:
    //
    //    Cannot handle this later, because stash:// will cause Qt to see the part after // as host,
    //    which will lower-case it (and thus invalidate the address).
    if(uri.startsWith("stash://", Qt::CaseInsensitive))
    {
        uri.replace(0, 7, "stash:");
    }
...
{
    QString ret = QString("stash:%1").arg(info.address);
    int paramCount = 0;

    if (info.amount)
    {
        ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::STASH, info.amount, false, BitcoinUnits::separatorNever));
        paramCount++;
    }
...
void openConfigfile()
{
    ...
    /* Open stash.conf with the associated application */
...
fs::path static StartupShortcutPath()
{
    ...
    if (chain == CBaseChainParams::MAIN)
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Stash Core.lnk";
    if (chain == CBaseChainParams::TESTNET) // Remove this special case when CBaseChainParams::TESTNET = "testnet4"
        return GetSpecialFolderPath(CSIDL_STARTUP) / "Stash Core (testnet).lnk";
    return GetSpecialFolderPath(CSIDL_STARTUP) / strprintf("Stash Core (%s).lnk", chain);
}

bool GetStartOnSystemStartup()
{
    // check for "Stash Core*.lnk"
...
fs::path static GetAutostartFilePath()
{
    std::string chain = ChainNameFromCommandLine();
    if (chain == CBaseChainParams::MAIN)
        return GetAutostartDir() / "stashcore.desktop";
    return GetAutostartDir() / strprintf("stashcore-%s.lnk", chain);
}
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    else
    {
        ...
        if (!optionFile.good())
            return false;
        std::string chain = ChainNameFromCommandLine();
        // Write a stashcore.desktop file to the autostart directory:
        optionFile << "[Desktop Entry]\n";
        optionFile << "Type=Application\n";
        if (chain == CBaseChainParams::MAIN)
            optionFile << "Name=Stash Core\n";
        else
            optionFile << strprintf("Name=Stash Core (%s)\n", chain);
...
LSSharedFileListItemRef findStartupItemInList(LSSharedFileListRef list, CFURLRef findUrl)
{
    // loop through the list of startup items and try to find the Stash Core app
...
bool SetStartOnSystemStartup(bool fAutoStart)
{
    ...
    if(fAutoStart && !foundItem) {
        // add Stash Core app to startup item list

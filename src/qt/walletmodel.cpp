...
CAmount WalletModel::getWatchShieldedBalance() const
{
    return wallet->GetWatchShieldedBalance();
}

UniValue WalletModel::shieldedMergeToAddress(const std::string& destaddress, CCoinControl *coinControl, CAmount nFee, const std::string memo, int nUTXOLimit, int nNoteLimit)
{
    return wallet->ShieldedMergeToAddress(destaddress, coinControl, nFee, memo, nUTXOLimit, nNoteLimit);
}

UniValue WalletModel::shieldedSendMany(const std::string& fromaddress, std::vector<CWallet::ShieldedRecipient> recipients, CAmount nFee)
{
    return wallet->ShieldedSendMany(fromaddress, recipients, nFee);
}
...
void WalletModel::checkBalanceChanged()
{
    ...
    CAmount newShieldedBalance = getShieldedBalance();
    CAmount newAnonymizedBalance = getAnonymizedBalance();
    CAmount newWatchOnlyBalance = 0;
    CAmount newWatchUnconfBalance = 0;
    CAmount newWatchImmatureBalance = 0;
    CAmount newWatchShieldedBalance = 0;
    if (haveWatchOnly())
    {
        newWatchOnlyBalance = getWatchBalance();
        newWatchUnconfBalance = getWatchUnconfirmedBalance();
        newWatchImmatureBalance = getWatchImmatureBalance();
        newWatchShieldedBalance = getWatchShieldedBalance();
    }
    ...
    if(cachedBalance != newBalance || cachedUnconfirmedBalance != newUnconfirmedBalance || cachedImmatureBalance != newImmatureBalance ||
		...
		cachedWatchShieldedBalance != newWatchShieldedBalance)
    {
        ...
        cachedWatchShieldedBalance = newWatchShieldedBalance;
        Q_EMIT balanceChanged(newBalance, newUnconfirmedBalance, newImmatureBalance, newAnonymizedBalance, newShieldedBalance,
                            newWatchOnlyBalance, newWatchUnconfBalance, newWatchImmatureBalance, newWatchShieldedBalance);
    }
}
...
WalletModel::SendCoinsReturn WalletModel::prepareTransaction(WalletModelTransaction &transaction, const CCoinControl& coinControl)
{
    ...
    for (const SendCoinsRecipient &rcp : recipients)
    {
        ...
        {   // User-entered stash address / amount:
...
WalletModel::SendCoinsReturn WalletModel::sendCoins(WalletModelTransaction &transaction)
{
...

    {
        ...
        for (const SendCoinsRecipient &rcp : recipients)
        {
            ...
            else if (!rcp.message.isEmpty()) // Message from normal stash:URI (stash:XyZ...?message=example)
            {

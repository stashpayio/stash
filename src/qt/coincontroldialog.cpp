...
void CoinControlDialog::updateLabels(WalletModel *model, QDialog* dialog)
{
    ...
    BOOST_FOREACH(const COutput& out, vOutputs) {
        ...
        // Priority
        dPriorityInputs += (double)out.tx->tx->vout[out.i].nValue * (out.nDepth+1);dPriorityInputs += (double)out.tx->tx->vout[out.i].nValue * (out.nDepth+1);
    ...
    int nDisplayUnit = BitcoinUnits::STASH;
    ...
    BOOST_FOREACH(const PAIRTYPE(QString, std::vector<COutput>)& coins, mapCoins) {
        ...
        BOOST_FOREACH(const COutput& out, coins.second) {
            ...
            if(ExtractDestination(out.tx->tx->vout[out.i].scriptPubKey, outputAddress))
            {
                ...
                // if listMode or change => show stash address. In tree mode, address is not shown again for direct wallet address outputs

...
{
    fs::path path;
    if (gArgs.IsArgSet("-exportdir")) {
        path = fs::system_complete(gArgs.GetArg("-exportdir", ""));
...
std::string CopyrightHolders(const std::string& strPrefix, unsigned int nStartYear, unsigned int nEndYear)
{
    ...
    // Check for untranslated substitution to make sure Stash Core copyright is not removed by accident
    if (strprintf(COPYRIGHT_HOLDERS, COPYRIGHT_HOLDERS_SUBSTITUTION).find("Stash Core") == std::string::npos) {
        strCopyrightHolders += "\n" + strPrefix + strprintf(" %u-%u ", 2018, nEndYear) + "The Stash Core developers";
    }

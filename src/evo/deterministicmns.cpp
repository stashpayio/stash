...
bool CDeterministicMNManager::IsDIP3Enforced(int nHeight)
{
    ...
    return nHeight >= sporkManager.GetSporkValue(SPORK_15_DETERMINISTIC_MNS_ENABLED);
...
bool CDeterministicMNManager::IsDIP3Active(const CBlockIndex* pindexPrev)
{
    return VersionBitsState(pindexPrev, Params().GetConsensus(), Consensus::DEPLOYMENT_DIP0003, versionbitscache) == THRESHOLD_ACTIVE;
}
...
void CDeterministicMNManager::UpgradeDBIfNeeded()
{
    ...
    // Should always be the new version (we skipped old version)
    throw std::runtime_error("Excepted version b_b2 but got b_b");

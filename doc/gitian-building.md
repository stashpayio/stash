...
- [Building Stash Core](#building-stash-core)
...
Clone the git repositories for Stash Core and Gitian.
...
in the Stash Core repository under 'Fetch and create inputs' to install sources which require
...
To build Stash Core (for Linux, OS X and Windows) just follow the steps under 'perform
Gitian builds' in [doc/release-process.md](release-process.md#perform-gitian-builds) in the Stash Core repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/stash/.git/
    remote: Counting objects: 57959, done.
    remote: Total 57959 (delta 0), reused 0 (delta 0), pack-reused 57958
    Receiving objects: 100% (57959/57959), 53.76 MiB | 484.00 KiB/s, done.
    Resolving deltas: 100% (41590/41590), done.
    From https://github.com/stashpayio/stash
...
For example:
```bash
URL=https://github.com/crowning-/stash.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit stash=${COMMIT} --url stash=${URL} ../stash/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit stash=${COMMIT} --url stash=${URL} ../stash/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit stash=${COMMIT} --url stash=${URL} ../stash/contrib/gitian-descriptors/gitian-osx.yml
```
...
and the stash git repository with the desired tag must both be available locally, and then gbuild must be
...
LXC_ARCH=amd64 LXC_SUITE=trusty on-target -u root \
  ...
  $( sed -ne '/^packages:/,/[^-] .*/ {/^- .*/{s/"//g;s/- //;p}}' ../stash/contrib/gitian-descriptors/*|sort|uniq )
...
BTCPATH=/some/root/path/stash
SIGPATH=/some/root/path/stash-detached-sigs

./bin/gbuild --url stash=${BTCPATH},signature=${SIGPATH} ../stash/contrib/gitian-descriptors/gitian-win-signer.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/stash-linux-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/stash-win-build.assert
    gpg --detach-sign ${VERSION}-osx-unsigned/${SIGNER}/stash-osx-build.assert
```
...
[stash/gitian.sigs](https://github.com/stashpay/gitian.sigs/) repository, or if that's not possible to create a pull
request.

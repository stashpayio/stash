...
### Send 10000 STASH to New Addresses

Send exactly 10000 STASH to each new address created above.
...
Copy the masternode private key and correspondig collateral output transaction that holds the 10000 STASH.

*Note: The masternode priviate key is **not** the same as a wallet private key. **Never** put your wallet private key in the masternode.conf file. That is almost equivalent to putting your 10000 STASH on the remote server and defeats the purpose of a hot/cold setup.*
...
[From the stash github repo](https://github.com/stashpayio/stash/blob/master/doc/masternode_conf.md)
...
## Update stash.conf on server

If you generated a new masternode private key, you will need to update the remote `stash.conf` files.

Shut down the daemon and then edit the file.

```nano .stashcore/stash.conf```

### Edit the masternodeprivkey
If you generated a new masternode private key, you will need to update the `masternodeprivkey` value in your remote `stash.conf` file.
...
and comparing with the official explorer at https://explorer.stashpay.io/chain/Stash
...
```
stash-cli masternode status
{
    "outpoint" : "<collateral_output>-<collateral_output_index>",
    "service" : "<ipaddress>:<port>",
    "pubkey" : "<10000 STASH address>",
...
Search your Masternodes on https://stashninja.pl/masternodes.html
